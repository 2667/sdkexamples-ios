                     ...
                     isIncoming:(BOOL)isIncoming
{
    ...
    if (self) {
        ...
        if ([ud valueForKey:kLocalCallBitrate]) {
            [_callSession setBitrate:[[ud valueForKey:kLocalCallBitrate] intValue]];
        }
...
{
    EMTextMessageBody *textBody = [[EMTextMessageBody alloc] initWithText:str];
    EMMessage *message = [[EMMessage alloc] initWithConversationID:_callSession.remoteUsername from:_callSession.username to:_callSession.remoteUsername body:textBody ext:nil];
    message.isRead = YES;
...
//        printf("buffeSize:%d,width:%d,height:%d,bytesPerRow:%d,bytesrow0 :%d,bytesrow1 :%d,bytesrow2 :%d\n",buffeSize,width,height,bytesPerRow,bytesrow0,bytesrow1,bytesrow2);
        ...
        [_callSession processPreviewData:(char *)bufferPtr width:width height:height];
...
{
    if(![_callSession.sessionId isEqualToString:aSession.sessionId]){
        return;
    }
                           ...
                           error:(EMError *)aError
{
    ...
    if(aError){
        ...
        UIAlertView *errorAlert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"error", @"Error") message:aError.description delegate:self cancelButtonTitle:NSLocalizedString(@"ok", @"OK") otherButtonTitles:nil, nil];
